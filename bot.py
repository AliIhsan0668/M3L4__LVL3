...
async def new_project(ctx):
    await ctx.send("Lütfen projenin adını girin!")
                                                                                                    #!new_projectle komut çalışır
    def check(msg):                                                                                 # id ve isim alınıp link eklenir
        return msg.author == ctx.author and msg.channel == ctx.channel                              # get_status ile veriler kullanıcıya aktarılır
                                                                                                    #kullanıcı seçimine göre durum data isimli listeye eklenir
    name = await bot.wait_for('message', check=check)                                               #insert_project(tuple(data)) diyerek ekleme gerçekleşir
...
async def get_projects(ctx):
    ...
    if projects:
        text = "\n".join([f"Project name: {x[2]} \nLink: {x[4]}\n" for x in projects])                                            #Tüm projeleri çağırır.
        await ctx.send(text)                                                                                                     #İsmi ve linki indexlerle kullanıya aktarır
    else:
...
async def skills(ctx):
    ...
    projects = manager.get_projects(user_id)
    if projects:
        projects = [x[2] for x in projects]                                                         #skill olanakları gözükür proje ismi alınır
        await ctx.send('Bir beceri eklemek istediğiniz projeyi seçin')                              #bir seçenek yazılır ve projects teki projeye eklenir
        await ctx.send("\n".join(projects))
...
async def delete_project(ctx):
    ...
    if projects:
        ...
        await ctx.send("Silmek istediğiniz projeyi seçin")                                  #Hangi projenin silineceğini belirlenir
        await ctx.send("\n".join(projects))                                                 #projects name kullanılarak proje silinir
...
async def update_projects(ctx):
    user_id = ctx.author.id                                                                     #keyleri üzerinden projede neyi değiştireceğimizi seçiyoz
    projects = manager.get_projects(user_id)                                                    #update_projects metodu ile güncelleme yapılır
    if projects:                                                                                #
        projects = [x[2] for x in projects]
